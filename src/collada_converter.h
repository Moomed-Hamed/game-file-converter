...
void convert_collada(const char* path, const char* binary_name, const char* text_name)
{
	...
	Mesh_Data final_mesh = make_mesh_data(num_dae_vertices,
...
void convert_collada_anim(const char* path, const char* binary_name, const char* text_name, const char* anim_name)
{
	File_Reader_Collada file = {};
	file.read_file(path);
	...
	// Library Geometries : positions, normals, faces, vertices
	{
		...
		file.seek_tag("triangles");
		sscanf(file.read_ptr, "%*[^=] %*[a-z=\"] %d", &num_dae_faces);
		num_dae_vertices = num_dae_faces * 3;
		...
		for (int i = 0; i < num_dae_vertices; i++)
		{
			dae_vertices[i].x = file.parse_int(); // print("position : %d ", dae_vertices[i].x);
			dae_vertices[i].y = file.parse_int(); // print("normal : %d \n", dae_vertices[i].y);
			for (int i = 0; i < stride - 2; i++) file.parse_int(); // skip unwanted info
		}
	}
	...
	// Library Visual Scenes : bone heirarchy, local transforms
	{
		...
		{
			int dummy;
			char* bone_name = file.parse_node(&dummy, 64);
			if (strcmp(bone_name, bone_names[0]) == 0) break;
		} //print("found root bone\n");
	...
	Mesh_Data_Anim final_mesh = make_mesh_data(num_dae_vertices,
	...
	save_animation_data(animations, bones, num_bones, num_keyframes, anim_name, "animations.txt");

	print("\nfinished converting '%s'\n", path);
}

// the insanely difficult one with textures
void convert_collada_anim_uv(const char* path, const char* binary_name, const char* text_name, const char* anim_name)
{
	File_Reader_Collada file = {};
	file.read_file(path);

	//mesh
	int num_dae_positions = -1, num_dae_normals = -1, num_dae_textures = -1, num_dae_vertices = -1, num_dae_faces = -1;
	vec3*  dae_positions = NULL;
	vec3*  dae_normals   = NULL;
	vec2*  dae_textures  = NULL;
	ivec3* dae_vertices  = NULL; // vertex = index of position & normal
	...
	// Library Geometries : positions, normals, faces, vertices
	{
		...
		dae_positions = (vec3*)file.parse_float_array(&num_dae_positions); num_dae_positions /= 3;
		dae_normals   = (vec3*)file.parse_float_array(&num_dae_normals);   num_dae_normals   /= 3;
		dae_textures  = (vec2*)file.parse_float_array(&num_dae_textures);  num_dae_normals   /= 2;

		file.seek_tag("triangles"); char wtf[256] = {};
		sscanf(file.read_ptr, " %[^0-9] %d", wtf, &num_dae_faces);
		num_dae_vertices = num_dae_faces * 3;
		...
		dae_vertices = (ivec3*)malloc(num_dae_vertices * sizeof(ivec3));

		for (int i = 0; i < num_dae_vertices; i++)
		{
			dae_vertices[i].x = file.parse_int(); // print("position : %d ", dae_vertices[i].x);
			dae_vertices[i].y = file.parse_int(); // print("normal : %d ", dae_vertices[i].y);
			dae_vertices[i].z = file.parse_int(); // print("texture  : %d \n", dae_vertices[i].z);
		}
	}
	...
	// save the animated mesh
	Mesh_Data_Anim_UV final_mesh = make_mesh_data(num_dae_vertices,
		dae_positions, dae_normals, dae_textures, weights, bone_ids, dae_vertices);
